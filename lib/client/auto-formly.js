...
class autoFormly extends autoFormlyHelpers {
     ...
     * @param {array|object} fields - schema keys you want to use or formly fields configuration (optional)
     * @returns {array}
     */
    ...
    filterSchema(schema, fields) {
        if (!angular.isArray(fields) && !angular.isObject(fields)) {
            throw this.createError("Fields have to be an array or object");
        }
        ...
        _.each(fields, (value, key) => {
            ...
            if (!schema.schema(id)) {
                throw this.createError(`There is no '${id}' in schema`);
            }

            // check if hidden 
            if (config !== false) {
                sorted[id] = schema.schema(id);
            }

            // extend formly field configuration
            if ("object" === typeof config) {
                sorted[id].autoformly = angular.merge({}, sorted[id].autoformly, config);
            }
        });
